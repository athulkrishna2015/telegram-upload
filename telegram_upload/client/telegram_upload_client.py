...
class TelegramUploadClient(TelegramClient):
    ...
    async def _get_sender(self):
        ...
        # Round robin
        sender = self._sender_pool.pop(0)
        self._sender_pool.append(sender)
        return sender
    ...
    async def _send_topic_message(self, entity, message, reply_to):
        ...
        message, entities = await self._parse_message_text(message, None)
    ...
    async def _send_album_media_resilient(self, entity, media, reply_to=None):
        try:
            return await self._send_album_media(entity, media, reply_to=reply_to)
        except MediaEmptyError:
            if len(media) <= 1:
                click.echo('Skipping an invalid album item.', err=True)
                return []
            midpoint = len(media) // 2
            click.echo('Album batch rejected, retrying as smaller albums.', err=True)
            await self._send_album_media_resilient(entity, media[:midpoint], reply_to=reply_to)
            await self._send_album_media_resilient(entity, media[midpoint:], reply_to=reply_to)
            return []
        ...
        try:
            try:
                ...
                if send_as_media:
                    ...
                    message = self._send_file_message(entity, file, thumb, progress, reply_to=reply_to)
            finally:
                bar.render_finish()
    ...
    async def _send_file_part(self, request: TLRequest, part_index: int, part_count: int, pos: int, file_size: int,
        ...
        :param sender: Sender to use for this part. Optional.
        :return: None
        """
        result = None
